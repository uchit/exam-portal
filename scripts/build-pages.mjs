...
write("about.html", staticPage({
  ...
  bodyHtml: `    <div class="prose page-head">
      ...
      <p>Every practice mode, the full mock exam, the diagnostic, the glossary, and the quick reference are free with no account or sign-up required. By default, your personal progress, flags, and history are saved in your browser only. When you finish a mock exam, an anonymous, aggregate result (pass/fail, score, per-domain breakdown — no identifiers of any kind) is added to the community stats below. Answering any question, anywhere on the site, also anonymously increments a shown/correct counter for that specific question — no identifiers, just a running tally we use to catch questions that are miscalibrated or have a weak wrong answer.</p>
      ...
      <p>Separately, on the Progress page, you can optionally tell us how your real exam went (pass/fail, and a scaled score if you want to share it) — entirely anonymous, not tied to your account or any identifier, and only used to show how predictive practice accuracy here actually is of a real pass. It's opt-in and dismissible; declining changes nothing about how the site works.</p>
